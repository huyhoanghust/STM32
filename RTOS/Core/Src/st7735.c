...
- Cau Hinh SPI, Sua lai marco de dung voi SPI can dung: #define st7735_PORT hspi1
Pinout:
...
//*********************** Marco for HAL LIB ***************************************************///
// #ifdef USE_HAL_LIB 
#define st7735_PORT hspi2
extern SPI_HandleTypeDef st7735_PORT;
/****************** Low Level Functions **********************************************************************/	
void st7735_Trans(uint8_t *buff, uint16_t buff_size)
{
	HAL_SPI_Transmit(&st7735_PORT, buff, buff_size, HAL_MAX_DELAY);
}
...
{
    ST7735_CMD();
		st7735_Trans(&cmd, sizeof(cmd));
}
void ST7735_WriteData(uint8_t* buff, size_t buff_size) 
{
    ST7735_DATA();
    st7735_Trans(buff, buff_size);
}
...
void ST7735_WriteData8(uint8_t data) 
{
    ST7735_DATA();
		uint8_t data8[1];
		data8[0] = data;
    st7735_Trans(data8, sizeof(data));
}
void ST7735_WriteData16(uint16_t data) 
{
    ST7735_DATA();
		...
		data8[0] = data>>8 & 0xFF;
		data8[1] = data &0xFF;
    st7735_Trans(data8, sizeof(data8));
}
